...
function convertToApiData(
  ...
  export default function dataProcessing(items) {
    ...
    for (let i in items.processedmeat) {
      ...
      processedMinute = [...processedMinute, items.processedmeat[i].sensory_eval?.deepaging_data?.minute || 0];
      processedDataImgPath = [...processedDataImgPath, items.processedmeat[i].sensory_eval?.imagePath || "null"];
    }
